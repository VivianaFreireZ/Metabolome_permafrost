...
Sample_Name = "bNTI_feat_microbe_within_fen" # Input sample name
Factor = "Habitat"
Level = "fen"
...
tree <- read.tree('bac_arc_tree_comb.tree')

meta <-  read_csv("metadata_matched_MAGs_data.csv") %>% 
  select(c(SampleID, Month, Habitat, Core, Depth, Year, Sampletype, names_bnti)) %>% 
  rename(SampleID_1 = names_bnti)

data <- read_csv('otu_normalized.csv') %>% 
...
otu_fen <- data[rownames(data) %in% meta_fen$SampleID,]
otu_fen <- otu_fen[, colSums(otu_fen) > 0]
...
for(rep in range){
  ...
  if(!dir.exists(paste0("/home/u1/vfreirezapata/ondemand/data/Meta-Metabolome_Ecology/Paper_figures_2022/input/microbes/Feature_fen", "/"))){
    dir.create(paste0("/home/u1/vfreirezapata/ondemand/data/Meta-Metabolome_Ecology/Paper_figures_2022/input/microbes/Feature_fen", "/"))
  }
  
  if(!dir.exists(paste0("/home/u1/vfreirezapata/ondemand/data/Meta-Metabolome_Ecology/Paper_figures_2022/input/microbes/Feature_fen", "/", Factor, "-", Level))){
    dir.create(paste0("/home/u1/vfreirezapata/ondemand/data/Meta-Metabolome_Ecology/Paper_figures_2022/input/microbes/Feature_fen", "/", Factor, "-", Level))
  }
  
  write.csv(null.by.samp, paste0("/home/u1/vfreirezapata/ondemand/data/Meta-Metabolome_Ecology/Paper_figures_2022/input/microbes/Feature_fen", "/", Factor, "-", Level, "/", 
