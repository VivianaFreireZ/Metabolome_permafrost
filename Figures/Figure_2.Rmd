...
gg_network <- imap(network_graph, function(x, y){
  ...
  ggraph(gg, layout = 'fr') +
    ...
    scale_shape_manual(values = c('Met. Cluster' = 15, 'Bacteria' = 16, 'Env. Factor' = 17),
                       drop = FALSE) +
    new_scale_color() +
    ...
    geom_node_label(aes(label = label), hjust = 0.5, nudge_y = -0.5, repel = TRUE, size = 1.8) +
    labs(title = y,
         shape = 'Node\nType',
...
```{r}
figure_2 <- (gg_network_final/ legend / (idx_plot | (contrib_plot / class_plot))) +
...
ggsave('output/Figure_2.png', figure_2, dpi = 300,  width = 215, 
       height = 180, units = 'mm')
```
