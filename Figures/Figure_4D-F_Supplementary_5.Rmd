...
```{r}
metaT_all <- read_tsv('../input/Emerge_metaTs_processed_v7/metaT_pathways.tsv') %>% 
  filter(pathway == 'CAZy',
...
cazymes_hits <- from_dram %>% 
  ...
  mutate(family = str_remove(cazy_id, '_.*'),
         sub_family = str_extract(cazy_id, '(?<=_).*')) %>% 
  group_by(fasta) %>%
  mutate(num_cazy = n()) %>% 
  distinct() %>% 
  group_by(fasta, num_cazy) %>% 
  summarise(num_family = n()) %>% 
