### From https://github.com/danczakre/betaNTI-feature/blob/main/bNTI_feature_by_sample_create-nulls_new.R
### Danzack et al 2022

### R-script to generate individual null bNTI(feature) reps
# Adapted from the "comdistnt" in the picante package by RE Danczak

Sample_Name = "bNTI_Feat_TWCD_bulk" # Input sample name
...
library(Rfast) # For faster variant of finding column minimum
library(tidyverse) # For left joining
...
data = read.csv("Processed_Abisko_Data_so_4-11-21.csv", row.names = 1) # Load in assemblage
tree = read.tree("Abisko_Weighted_All-Trans_UPGMA_so_4-11-21.tre") # Load in tree
...
phylo = match.phylo.data(tree, data) # Matching ICR dataset to the tree ## Not transpose needed for metabolite tree
...
for(rep in range){
  ...
  if(!dir.exists(paste("/home/u1/vfreirezapata/ondemand/data/Meta-Metabolome_Ecology/BNTI_feature_TWCD_8-2-22/Bulk_across/", Sample_Name, sep = ""))){
    dir.create(paste("/home/u1/vfreirezapata/ondemand/data/Meta-Metabolome_Ecology/BNTI_feature_TWCD_8-2-22/Bulk_across/", Sample_Name, sep = ""))
  }
  
  write.csv(null.by.samp, paste0("/home/u1/vfreirezapata/ondemand/data/Meta-Metabolome_Ecology/BNTI_feature_TWCD_8-2-22/Bulk_across/", Sample_Name, "/", Sample_Name, "_Feature_Samp_Null_rep", rep, ".csv"),
            quote = F, row.names = T)
