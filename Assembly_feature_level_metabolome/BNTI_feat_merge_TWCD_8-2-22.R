...
Sample_Name = "bNTI_feat_TWCD" # Input sample name
...
files = list.files(path = paste0("/home/u1/vfreirezapata/ondemand/data/Meta-Metabolome_Ecology/Paper_figures_2022/input/Peat_so/Feature_level_by_sample/bNTI_Feat_MCD_bulk/"),
...
# Joining with molecular infor data


mol <- read.csv("Processed_Abisko_Mol_so_4-11-21.csv") 

feat.samp.frame_mol  <- left_join(feat.samp.frame, mol, by = 'Mass')

write.csv(feat.samp.frame_mol, "output_MCD_3-22-22/Bulk_feature_MCD_mol_3-22-22.csv")
...
feat.melt_mol  <- left_join(feat.melt, mol, by = 'Var1') %>% 
  left_join(meta, by = c('Var2' = 'SampleID'))

write.csv(feat.melt_mol, "output_MCD_3-22-22/Bulk_most_variable_MCD_3-24-22.csv")
...
plot <- feat.melt_mol %>% 
  ...
  ggplot(aes(x = MolForm, y = value, color = Habitat)) +
  geom_point() +
...
ggsave("output_MCD_3-22-22/Bulk_most_variable_MOLFORM_plot_3-24-22.png", plot)
...
plot <- feat.melt_mol %>% 
  ...
  ggplot(aes(x = bs2_class, y = value, color = Habitat)) +
  geom_point() +
  geom_hline(yintercept = c(2, -2), color = 'red', linetype = 'dashed') + 
  #scale_y_continuous(limits = c(-6, 6)) +
  theme_bw() +
...
ggsave("output_MCD_3-22-22/Bulk_most_variable_CLASS_plot_3-24-22.png", plot)
...
plot <- feat.melt_mol %>% 
  ...
  ggplot(aes(x = El_comp, y = value, color = Habitat)) +
  geom_point() +
  geom_hline(yintercept = c(2, -2), color = 'red', linetype = 'dashed') + 
  #scale_y_continuous(limits = c(-6, 6)) +
  theme_bw() +
...
ggsave("output_MCD_3-22-22/Bulk_most_variable_ELMENTAL_plot_3-22-22.png", plot)
